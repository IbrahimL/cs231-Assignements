...
def affine_backward(dout, cache):
    ...
    #dout = dout.reshape((x.shape[0], w.shape[1]))
    ###########################################################################
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        cache = x, x_norm, gamma, beta, var, mean, eps
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
...
def batchnorm_backward(dout, cache):
    ...
    """
    x, x_norm, gamma, beta, var, mean, eps = cache
    m = x.shape[0]
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    dx_norm = dout * gamma
    dvar = np.sum(dx_norm*(x-mean)*(-0.5*np.power((var+eps), -3/2)), axis=0)
    dmean = np.sum(-dx_norm/np.sqrt(var+eps), axis=0) + (1/m)*dvar*np.sum(-2*(x-mean), axis=0)
    dx = dx_norm/np.sqrt(var+eps) + (2/m)*dvar*(x-mean) + (1/m)*dmean
...
def batchnorm_backward_alt(dout, cache):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    dx_norm = dout * gamma
    dx = (C/m) * (m*dx_norm-np.sum(dx_norm, axis=0)-x_norm*np.sum(dx_norm*x_norm, axis=0))
    dgamma = np.sum(dout*x_norm, axis=0)
    dbeta = np.sum(dout, axis=0)
        ...
        for j in range(C):
            for k in range(Ho):
                for l in range(Wo):
                    ...
                    dx[i, j, k*stride+ind[0], l*stride+ind[1]] = dout[i, j, k, l]
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    out, cache = batchnorm_forward(x.swapaxes(0, 1).reshape(C, -1).T, gamma, beta, bn_param)
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    cache = x, x_norm, gamma, beta, var, mean, eps, G
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def spatial_groupnorm_backward(dout, cache):
    ...
    x, x_norm, gamma, beta, var, mean, eps, G = cache
    C = 1 / np.sqrt(var + eps)
    N, C, H, W = dout.shape
